        ...
        char* interface/* lent */)
{
  ...
  if(ntohs(arpHdr->ar_op) == arp_op_reply){
    ...
    /*
    struct sr_arpentry* req = sr->cache->requests;
    struct sr_packet* reqPkt;
    while(req != NULL){
...
//send_packet(sr, buf, len, interface);

}/* end sr_ForwardPacket */


void send_packet(struct sr_instance* sr, uint8_t* packet, unsigned int len, char* interface){
  ...
  free(ent);
    // send_arp_req(sr, iface, ethHdr->ether_shost, ipHdr->ip_src, ipHdr->ip_dst);
